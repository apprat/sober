...
1. 键盘访问支持：支持 `Tab` 向下选中焦点，使用 `shift` + `Tab` 向上切换焦点，使用空格或回车触发点击事件，使用方向键增加/减少进度。

2. CSS 优化：引入了 PostCSS 预处理器，源代码更简洁易读。

3. Material 3 Expressive 设计支持：添加了新的 M3 组件以及移除了规范废弃的组件。
...
6. 组件增强：大多数组件都获得了不同程度的增强和新的属性支持。
