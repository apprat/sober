...
svg{
  ...
  path{
    ...
    animation: shapeAnim 6s cubic-bezier(0.5, 0.2, 0, 0.8) infinite, rotateAnim 6s cubic-bezier(0.5, 0.2, 0, 0.8) infinite;
  }
}
@keyframes shapeAnim {
  0% { d: path("${pathFrame}");}
...
export class Loading extends useElement({
  props,
  style: support ? [style, animeStyle] : [style],
  template
}) {
