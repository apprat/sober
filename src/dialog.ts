...
const alert = (options: {
  ...
  dialog.size = options.size ?? 'basic'
...
export default class Component extends defineElement({
  ...
  setup() {
    ...
    const negative = ref()
    const positive = ref()
    const show = () => wrapper.target.classList.add('show')
    const dimiss = () => wrapper.target.classList.remove('show')
    const onShow = () => {
      this.dispatchEvent(new Event('show'))
      show()
    }
    const onDimiss = (code: EventCode) => {
      if (!this.dispatchEvent(new CustomEvent('dimiss', { detail: { code }, cancelable: true }))) return
      dimiss()
    }
    const onIconSlotChange = () => {
      icon.target.assignedElements().length > 0 ? wrapper.target.classList.add('icon') : wrapper.target.classList.remove('icon')
    }
    return {
      expose: { show, dimiss },
      watches: {
      ...
      render: () => html`
        <style>${style}</style>
        <slot name="trigger" @click="${onShow}"></slot>
        <div class="wrapper" ref="${wrapper}">
          <div class="scrim" @click="${() => onDimiss(EventCode.SCRIM)}"></div>
          <div class="wrapper-container">
            <div class="container">
              <div class="header">
                <slot name="icon" ref="${icon}" @slotchange="${onIconSlotChange}"></slot>
                <slot name="headline"></slot>
                <div class="action">
                  <s-ripple class="text-button" ref="${negative}" @click="${() => onDimiss(EventCode.NEGATIVE_BUTTON)}"></s-ripple>
                  <s-ripple class="text-button" ref="${positive}" @Click="${() => onDimiss(EventCode.POSITIVE_BUTTON)}"></s-ripple>
                </div>
              </div>
              <slot></slot>
            </div>
